...
export const API_CANCEL_ORDER = async(publicKey, marketAddress, openOrdersAddress, clientOrderId, clob, priorityFee=0.005) => {
  let request = await fetch("https://v4.prism.ag/cancelOrder", {
    method: "POST",
    headers: {
...
export const API_CANCEL_ORDER_ALL = async(publicKey, marketAddress, openOrdersAddress, clob, priorityFee=0.005) => {
  let request = await fetch("https://v4.prism.ag/cancelAll", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
...
export const API_SETTLE_FUNDS = async(publicKey, marketAddress, openOrdersAddress, clob, priorityFee=0.005) => {
...
export const API_GET_UNSETTLED_BALANCES = async(address, clob) => {
  let response;
  try {
    let request = await fetch("https://v4.prism.ag/getUnsettledBalances?pubkey="+address+"&clob="+clob);
    response = await request.json();
  } catch (e) {
    response = {"success":true,"unsettled":[],"clob":"v2"}
  }
  return response;
}
...
export const batchOrderTransaction = async (side, orders, market, pubkey, clob="v2", priorityfee=0.005) => {

  let request = await fetch("https://v4.prism.ag/batch", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
  ...
  console.log("[openbook] batch", request);
  if(request.success) {
    const swapTransactionBuf = Buffer.from(request.transaction, 'base64');
    let tx = Transaction.from(swapTransactionBuf);
    return {
